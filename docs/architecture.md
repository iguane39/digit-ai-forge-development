...
| `gates/mutation_gate.py` (TF-0103.2) | — | Lit `mutants/mutmut-cicd-stats.json` (export mutmut) et applique le seuil (75 %, doc. dans le module) — 3e métrique du double gate, INFORMATIVE en CI pour v0 | `mutmut` (Linux/CI/devcontainer/WSL — jamais Windows natif) | l'exécution de mutmut elle-même |
